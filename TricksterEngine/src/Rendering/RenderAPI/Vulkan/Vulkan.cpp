...
{
    m_ModelMatrices.push_back({ a_ModelName, a_ModelMatrix });
}

void Vulkan::DrawModelImp(const _DrawInfo& a_DrawInfo)
{
    if(m_Models.count(a_DrawInfo.modelName) == 0)
    {
        LoadModel(a_DrawInfo.modelName);
    }
    auto& model = m_Models[a_DrawInfo.modelName];
    ...
    ubo.model = a_DrawInfo.modelMatrix;
	...
	vkCmdBindDescriptorSets(m_Command.buffers[m_CurrentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, m_Pipeline.layout, 0, 1, &model->texture.descriptor.sets[0], 0, nullptr);
    vkCmdDrawIndexed(m_Command.buffers[m_CurrentFrame], static_cast<uint32_t>(model->indices.size()), 1, 0, 0, 0);
        
}


void Vulkan::LoadSprite(const std::string& a_SpriteName)
{
    if (m_Sprites.count(a_SpriteName) == 0)
    {
        m_Sprites.insert({ a_SpriteName, std::make_unique<TricksterSprite>() });
        auto& sprite = m_Sprites[a_SpriteName];
        sprite->Setup(a_SpriteName);
        LOG("[Vulkan] Loaded sprite " + a_SpriteName);
    }
    else
    {
        //Already loaded
        LOG_USELESS("[Vulkan] Already loaded this model: " + a_SpriteName);
    }
}

void Vulkan::DrawSprite(const std::string& a_SpriteName, const glm::mat4& a_ModelMatrix, const bool a_IsScreenSpace)
{
    m_SpriteMatrices.push_back({ a_SpriteName, a_ModelMatrix, a_IsScreenSpace });
}

void Vulkan::DrawSpriteImp(const _DrawInfo& a_DrawInfo)
{
    if (m_Sprites.count(a_DrawInfo.modelName) == 0)
    {
        LoadSprite(a_DrawInfo.modelName);
    }
    auto& sprite = m_Sprites[a_DrawInfo.modelName];


    //This is the draw function for a model
    VkBuffer vertexBuffers[] = { sprite->VertexBuffer.get };
    VkDeviceSize offsets[] = { 0 };
    UniformBufferObject ubo{};

    auto camera = MeshManager::GetInstance()->GetCamera();
    ubo.model = a_DrawInfo.modelMatrix;
    if (a_DrawInfo.isScreenSpace)
    {
        ubo.view = glm::identity<glm::mat4>();
        ubo.projection = glm::ortho(0.f, 1.f, 1.f, 0.f);
    }
    else
    {
    ...
    vkCmdBindIndexBuffer(m_Command.buffers[m_CurrentFrame], sprite->IndexBuffer.get, 0, VK_INDEX_TYPE_UINT32);
    vkCmdBindDescriptorSets(m_Command.buffers[m_CurrentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, m_Pipeline.layout, 0, 1, &sprite->texture.descriptor.sets[0], 0, nullptr);
    vkCmdDrawIndexed(m_Command.buffers[m_CurrentFrame], 6, 1, 0, 0, 0);
