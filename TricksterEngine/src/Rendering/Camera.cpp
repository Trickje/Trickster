...
Camera::Camera()
{
	EventManager::GetInstance()->GameLoopEvents.OnUpdate.AddListener(std::bind(&Camera::RecalculateViewProjection, this));
	m_View = glm::mat4(0.f);
...
glm::mat4  Camera::CalculateProjection()
{
	...
	m_Projection[3][3] = 0;
	*/
	m_Projection = glm::perspective(m_FOV, static_cast<float>(Application::Get()->GetWindow()->GetWidth()) / static_cast<float>(Application::Get()->GetWindow()->GetHeight()), m_Near, m_Far);
	m_Projection[1] *= -1;
	return m_Projection;
}