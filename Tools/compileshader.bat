...
echo:
echo:
for /r %FetchDir% %%i in (*.frag) do (%VULKAN_SDK%\Bin\glslc.exe -c %%i -o %OutputDir%%%~nifrag.spv
echo Compiled %%~ni.frag to %%~nifrag.spv
set /A "count=count+1") 
for /r %FetchDir% %%i in (*.vert) do (%VULKAN_SDK%\Bin\glslc.exe -c %%i -o %OutputDir%%%~nivert.spv
